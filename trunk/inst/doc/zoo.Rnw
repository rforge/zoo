...
library(quantreg)
library(fBasics)
online <- TRUE ## if set to FALSE the local copy of LU.rda
...
can be fit easily using the standard \code{lm} function and just insulating
\code{I(formula)} in the corresponding call.
\footnote{In addition to \code{lm} and \code{glm}, this approach works for
many other regression functions including
