...
since 1970-01-01 to class \code{"Date"}, used in the above.
...
na.contiguous(z1)
na.approx(z1)
na.approx(z1, index(z1))
na.locf(z1)
@

As the above example illustrates, \code{na.approx} uses by default
an equidistant spacing of the observations which can be changed,
e.g., to the underlying time scale, by setting the second argument of
\code{na.approx}.
