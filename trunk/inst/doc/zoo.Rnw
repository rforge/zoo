...
\Abstract{
  ...
  handling. A sub-class \code{"zooreg"} embeds regular time series
...
with \code{NA}s. For strictly regular series with numeric index, the class can be switched
...
%FIXME% also comment on regular series
...
%FIXME% include coercion to and from "ts"
...
%FIXME%
Describe \code{rapply} and the specialized functions \code{rollmean}, \code{rollmed}
and \code{rollmax}.
