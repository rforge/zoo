...
%\VignetteDepends{zoo,tseries,fBasics,strucchange,DAAG}
...
\code{"irts"} and \code{"timeSeries"}. In particular, it bridges the gap
between regular and irregular time series by providing coercion with (almost)
...
binding are briefly highlighted in the corresponding subsection.
...
In addition, a \code{as.character()} method might improve printed output\footnote{If
a \code{as.character()} method is already defined, but gives not the desired
output for printing, then a \code{index2char()} method can be defined. This is a
generic convenience function used for creating character representation of the
...
which only uses vector/matrix-type subsetting if the index is of class
...
To subset with a numeric index, the index has to be either insulated in \code{I()}
like \code{z[I(i)]} or the subsetting function \code{[[} can be used \code{z[[i]]}
which always subsets with the index class.
...
with the index \code{order.by} and if it does an object of class \code{"zooreg"}
inheriting from \code{"zoo"} is returned. The command \code{zooreg()} takes mostly
the same arguments as \code{ts()} and the whole function is in fact almost identical
to \code{ts()}.\footnote{Only if \code{order.by}
is specified in the \code{zooreg()} call, then \code{zoo(x, order.by, frequency)}
is called.} The main difference between \code{zoo()} and \code{zooreg()} is that
the latter will only create numeric indexes whereas the former only requires that
the index can be coerced to a numeric vector (for checking its regularity).\footnote{An
...
and are discussed in Section~\ref{sec:combining}.}
...
<<zooreg5>>=
as.ts.zoo(zr1) #FIXME#
identical(zr2, as.zoo(as.ts.zoo(zr2)))
@
...
object, then the corresponding method tries to return a \code{"zooreg"}
...
dropping or trying to add a \code{"frequency"} attribute.
...
of \code{"yearmon"} data and completes the methods needed for conveniently 
working with monthly data in \pkg{zoo}. Note, that all of these methods are 
very simple and rather obvious (as can be seen in the \pkg{zoo} sources), but prove
very helpful in the following examples.

First, we transform the index of the regular series \code{zr2} created in Section~\ref{sec:zooreg}
