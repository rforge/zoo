...
\subsection[lm, glm, randomForest, e1071, MASS, quantreg: regression-style modelling with \code{zoo}]{\pkg{lm}, \pkg{glm}, \pkg{randomForest}, \pkg{e1071}, \pkg{MASS}, \pkg{quantreg}: regression-style modelling with \code{zoo}}

\code{zoo} provides a general mechanism for interfacing with regression
style functions that utilizes \code{AsIs} and \code{zoo} 
methods of \code{model.frame}.  This mechanism can be used with linear modelling
function \code{lm}, general linear modelling function \code{glm}
and many other regression-style functions found in various R packages
provided their code is sufficiently standardized.  Functions which can
be in this way include the \code{randomForest} ensemble approach
...
specify formulas to these functions provided that all variables are
\code{zoo} objects.  In this case \code{zoo} will automatically align the 
zoo objects
in case they have different overlapping time scales.  \code{lag}
and \code{diff} can also be used in the formulas and will also
result in appropriate alignment.
...
See the help page for \code{model.frame.zoo} for more information.
