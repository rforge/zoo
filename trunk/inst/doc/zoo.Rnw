...
\Abstract{
  ...
  handling.
  %FIXME% also mention regular time series series
}
...
few additional commands have to be learned. %FIXME% also mention regular series
...
%FIXME% also discuss \code{as.numeric()} and \code{as.character()}
...
the following. %FIXME% subsetting/indexing with index class
...
\subsection[The subclass "zooreg" for regular series]{The subclass \code{"zooreg"} for regular series}
\label{sec:zooreg}
...
and methods to \code{frequency}, \code{deltat}, \code{cycle}.

New generic function \code{is.regular(x, strict = FALSE)}.

Briefly mention coercion to \code{"ts"}, this enables direct use of functions
such as \code{acf}, \code{arima}, \code{stl} and so on as these methods coerce
to \code{"ts"} first. The result has to be coerced back to \code{"zoo"} if appropriate.
 
Probably also mention \code{yearmon} and \code{yearqtr} here... 
