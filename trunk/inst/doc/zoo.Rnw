...
\Abstract{
  ...
  class with methods for totally ordered observations, such as
...
\citep[\url{http://www.R-project.org/}]{Z:R:2004}
...
in package \pkg{fBasics} from the \pkg{Rmetrics} bundle (Wuertz, FIXME)
and \code{"irts"} in package \pkg{tseries} (Trapletti and Hornik, FIXME)
and the \proglang{S4} class \code{"its"} in package \pkg{its} (Heywood, FIXME).
...
\citep{Z-papers:Zeileis+Leisch+Hornik:2002}. Hence the package was called
...
\subsection[Plotting "zoo" objects]{Plotting \code{"zoo"} objects}
\label{sec:plot}
...
\subsection[Combining "zoo" objects]{Combining \code{"zoo"} objects}
\label{sec:merge}
...
\subsection[Extracting/replacing the value und/or index of "zoo" objects]{%
  Extracting/replacing the value und/or index of \code{"zoo"} objects}
\label{sec:window}
...
\bibliography{Z,sc,Z-papers,Z-reports}
