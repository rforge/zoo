...
(for checking its regularity) and when converted to numeric and multiplied by the
frequency it must be (equivalent to) an integer. Furthermore, adding/substracting
