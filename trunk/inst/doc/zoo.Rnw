...
Section~\ref{sec:summary} gives a few summarizing remarks.
...
\code{"index"} attribute in which the index is stored. Both the data \code{x}
...
objects are typically only useful for numeric observations \code{x}. In contrast, special
...
@
Note that \code{zoo} provides \code{as.Date} methods to convert days 
since January 1, 1970 to class Date, used in the above.
The argument to \code{as.Date} was used to generate random 
...
coerce the indexes.  It is the responsibility of the user to ensure
...
it can be replaced using \code{coredata<-}. Both are new generic functions
...
a \code{window} method and also adds a new generic 
...
\pkg{stats},
\pkg{glm}, \pkg{randomForest}, \pkg{e1071}, \pkg{MASS}, \pkg{quantreg},
\pkg{strucchange}, \pkg{tseries} and \pkg{fBasics} in this section.

\subsection[stats: regression-style modelling with "zoo"]{\pkg{stats}: regression-style modelling with \code{zoo}}

\code{zoo} provides a general mechanism for interfacing with regression
style functions.
This mechanism can be used with linear modelling
function \code{lm}, general linear modelling function \code{glm}
and certain other regression-style functions found in various R packages.
This works for regression functions whose code
is sufficiently similar to the pattern established by \code{lm}
and includes the \code{randomForest} ensemble approach
from the \pkg{randomForest}
package, the \code{svm} support vector machine function
from the \pkg{e1071} package, the \code{lqs} resistant regression
function from the
\pkg{MASS} package and \code{rq} quantile regression fucntion
from the \pkg{quantreg} package.  

\code{zoo} allows one to 
specify formulas to these functions in which \emph{all} variables
in the formula are
\code{zoo} objects.  For such formulas, \code{zoo} will align the 
zoo objects, if necessary.  \code{lag}
and \code{diff} can also be used in the formulas and zoo will
perform appropriate alignment as well.

In each case, the user simply places \code{I(...)} around the 
entire
formula to tell the system that the special processing 
just described should be invoked.   For example,

<<regression1>>=
	yz <- zoo(1:12)^2
	xz <- zoo(1:9)^2
	fz <- zoo(gl(2,5)) # zoo factor object

	# example using diff and lag -- note I(...)
	lm(I(lag(yz) ~ diff(xz) + fz))

	# same example using glm
	glm(I(lag(yz) ~ diff(xz) + fz))

	# same example using RandomForest from package RandomForest
	library(randomForest)
	set.seed(1) 
	randomForest(I(lag(yz) ~ diff(xz) + fz))
@

See the help page for \code{model.frame.zoo} for more examples
...
if(online) {
  LU <- get.hist.quote("LU",  origin = as.Date(0))
...
\code{zoo} has provided additional methods to \code{as.Date} which
this example uses.
...
in days starting from an \code{origin}, in this case specified to be the
