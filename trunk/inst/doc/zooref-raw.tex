...
%FIXME% also mention as.numeric and as.character

%% \multicolumn{2}{l}{\textbf{Creation of regular series}} \\
%% \code{zoo(x, order.by, frequency} & as above with freq\\
%% \code{zooreg} & as before but with numeric index, same interface as ts() \\
...
%% \code{is.regular} & \\
%% \code{frequency}, \code{deltat} & \\
%% \code{cycle} & \\


%% \multicolumn{2}{l}{\textbf{Rolling functions}} \\
%% \code{rapply} & rolling functions \\
%% \code{rollmean} & rolling mean, median and maxim are \code{rollmean}, \code{rollmedian} and
%%   \code{rollmax}, respectively
