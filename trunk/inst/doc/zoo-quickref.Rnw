...
This document provides a cookbook of common idioms often used in the 
\code{zoo} package.  
Additional information on the commands discussed and on other
commands, not discussed, can be found in the vignette, reference
card and help pages.
...
x <- get.hist.quote(instrument = "532285.BO",start="2004-01-01",end = "2004-12-31",
                    origin = as.Date(0), compression = "w", quote = "Adj")
x <- na.omit(as.zoo(x))
index(x) <- as.Date(index(x))
